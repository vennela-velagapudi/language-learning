...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    error_log("Adding course: Language = $language, Teacher ID = $teacher_id");
    ...
    if ($table_result->num_rows == 0) {
        ...
        $create_table = "CREATE TABLE teacher_courses (
        ...
        if ($conn->query($create_table) === TRUE) {
            error_log("Table teacher_courses created successfully");
        } else {
    ...
    $check_query = "SELECT * FROM teacher_courses WHERE teacher_id = ? AND language = ?";
    ...
    $insert_query = "INSERT INTO teacher_courses (teacher_id, language) VALUES ($teacher_id, '$language')";
